...
module.exports = async (req, res) => {
  ...
  if (!process.env.OPENAI_API_KEY) {
    const reply = `Hello! I'm CodeHelper AI. You asked: "${message}". The app is working, but no OpenAI API key is set. Please add OPENAI_API_KEY in Vercel environment variables.`;
  ...
  try {
    ...
    console.error(error);
    res.status(500).json({ reply: 'Sorry, I could not connect to AI. Check your OpenAI API key and settings.' });
  }
};